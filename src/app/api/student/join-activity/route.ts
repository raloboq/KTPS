...
export async function POST(request: Request) {
  ...
  try {
    ...
    const studentIdStr = cookieStore.get('studentId')?.value;
    const studentName = cookieStore.get('studentFullName')?.value;
    const studentUsername = cookieStore.get('studentUsername')?.value;
    const studentEmail = cookieStore.get('studentEmail')?.value;
    ...
    console.log('- studentId cookie:', studentIdStr);
    console.log('- studentFullName cookie:', studentName);
    console.log('- studentUsername cookie:', studentUsername);
    console.log('- studentEmail cookie:', studentEmail);
    ...
    try {
      ...
      // Verificar si el estudiante existe en la tabla student_users y crearlo si no existe
      const checkStudent = await client.query(
        `SELECT id FROM student_users WHERE moodle_user_id = $1`,
        [studentId]
      );
      
      let dbStudentId = studentId;
      
      if (!checkStudent.rowCount || checkStudent.rowCount === 0) {
        // El estudiante no existe, lo creamos
        const insertResult = await client.query(
          `INSERT INTO student_users (moodle_user_id, username, email, fullname)
           VALUES ($1, $2, $3, $4)
           RETURNING id`,
          [studentId, studentUsername || 'student_' + studentId, studentEmail || '', studentName]
        );
        
        dbStudentId = insertResult.rows[0].id;
        console.log(`Estudiante con ID ${studentId} creado en la tabla student_users, ID en DB: ${dbStudentId}`);
      } else {
        dbStudentId = checkStudent.rows[0].id;
        console.log(`Estudiante encontrado en la tabla student_users, ID: ${dbStudentId}`);
      }
      ...
      // Asignar estudiante a la sala usando el ID de student_users
      await client.query(`
        INSERT INTO student_room_assignments (student_id, room_id, status)
        VALUES ($1, $2, 'active')
      `, [dbStudentId, roomId]);
