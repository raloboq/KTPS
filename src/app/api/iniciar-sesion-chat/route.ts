...
export async function POST(request: Request) {
  try {
    const { id_room, nombre_usuario } = await request.json();
    const result = await sql`
      INSERT INTO sesiones_chat (id_room, nombre_usuario)
      VALUES (${id_room}, ${nombre_usuario})
      RETURNING id_sesion_chat
    `;
    return NextResponse.json({ id_sesion_chat: result.rows[0].id_sesion_chat });
  } catch (error) {
